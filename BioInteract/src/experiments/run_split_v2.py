"""
run_split.py — Train + evaluate BioInteract v2 on a single split.
Direct print, no logging framework, maximally robust on Windows.
...
Usage:
    python -m src.experiments.run_split_v2 --split random
    python -m src.experiments.run_split_v2 --split cold_drug
    python -m src.experiments.run_split_v2 --split cold_target
"""
import warnings
...
def main():
    ...
    P(f"=== Split: {split_type} (v2: FocalLoss + Morgan + GraphAug) ===")
    ...
    # Focal Loss (alpha=0.75 for 5% positive rate)
    criterion = FocalLoss(alpha=0.75, gamma=2.0)
    P(f"  loss=FocalLoss(alpha=0.75, gamma=2.0)")
