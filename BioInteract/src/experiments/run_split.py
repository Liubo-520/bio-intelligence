"""
run_split.py — Train + evaluate BioInteract on a single split.
Direct print, no logging framework, maximally robust on Windows.

Usage:
    python -m src.experiments.run_split --split random
    python -m src.experiments.run_split --split cold_drug
    python -m src.experiments.run_split --split cold_target
"""
import warnings
...
def main():
    ...
    for epoch in range(1, cfg['training']['epochs'] + 1):
        ...
        P(f"  E{epoch:03d} | loss={avg_loss:.4f} | val_auroc={auroc:.4f} | {dt:.1f}s")
...
if __name__ == '__main__':
    try:
        ...
        P(f"FATAL ERROR:\n{err_msg}")
        # Also write to a separate error file
        LOGS_DIR.mkdir(parents=True, exist_ok=True)
