"""
run_split_v3.py — Train + evaluate BioInteract v3 on a single split.

v3 = v1 architecture + Morgan FP dual-channel + refined training recipe.
Fixes v2 issues:  FocalLoss removed → BCE with pos_weight (proven in v1),
                   GraphAugmentation disabled (drop_node=0, drop_edge=0),
                   Label smoothing 0.05 for mild regularisation.

Key differences from v1:
    + Morgan FP dual-channel drug encoding    (NEW)
    + AdamW optimizer                         (improved)
    + Cosine warmup scheduler                 (improved)
    + Validation-selected F1 threshold applied unchanged to tests
    + Label smoothing 0.05                    (regularisation)

Key differences from v2:
    - No FocalLoss (BCE + pos_weight instead) — restores strong gradients
    - No GraphAugmentation                    — preserves drug structure
    + Label smoothing 0.05                    — mild regularisation

Usage:
    python -m src.experiments.run_split_v3 --split random
    python -m src.experiments.run_split_v3 --split cold_drug
    python -m src.experiments.run_split_v3 --split cold_target
"""
import warnings
...
def smooth_labels(labels, smoothing=0.05):
    ...
    positive → 1 - smoothing/2 = 0.975
    negative → smoothing/2     = 0.025
    """
...
def main():
    ...
    P(f"=== Split: {split_type} (v3: Morgan FP + BCE + no GraphAug + LabelSmooth) ===")
    ...
    # BCE with pos_weight (same as v1 — proven effective)
    ...
    P(f"  graph_aug=DISABLED (drop_node=0, drop_edge=0)")
