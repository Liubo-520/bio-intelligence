...
\begingroup\color{red}
We compare BioInteract against six published DTI baselines spanning sequence,
graph, attention, transformer, and bilinear interaction models:
...
The methods span sequence CNNs, molecular graphs, self-attention, transformer
interaction models, and bilinear attention (Table~\ref{tab:comparison}).
...
The current-release Target-ID-held-out reconstruction yielded AUROC~$=0.930$.
Our audit shows, however, that six exact-sequence groups crossed its training
and test partitions. This score is consequently reported as an entity-held-out
result, not as evidence of sequence-novel generalisation. Its difference from
the random-split value may reflect relatedness within the kinase-focused Davis
...
current-release reconstruction. The maximum test-to-training Morgan Tanimoto
similarity ranged from 0.196 to 0.538 (median 0.257), documenting
identifier-level compound exclusion while also making the remaining
chemical-neighbourhood relation explicit.
\endgroup

{\color{red}\subsection{Ablation Study}}

\begingroup\color{red}
We report the restored component-removal results in Table~\ref{tab:ablation} and
Fig.~\ref{fig:ablation}.  To keep the full-model entry consistent with the
current reproducible release, the BioInteract row uses the canonical current
entity-split metrics also reported in Table~\ref{tab:results}.  The
component-removed rows retain their original numerical values.  Consequently,
the displayed row-to-row differences are descriptive component-sensitivity
values, rather than causal effect estimates from a controlled matched-retraining
experiment.

\begin{table}[htbp]
\centering
\caption{Component-removal summary on the Davis dataset. The Full BioInteract
  row uses canonical current entity-split metrics, whereas the component-removed
  rows retain their original numerical values. Thus, differences across displayed
  rows are descriptive and are not controlled matched-retraining effect estimates.}
\label{tab:ablation}
\begin{tabular}{lcccc}
\toprule
...
\caption{\textbf{Component-removal summary.} Panel A shows the random- and
  Target-ID-held-out metric matrix. Panel B reports each component-removed row's
  AUROC difference from the displayed full-model row. Because the full-model row
  uses current canonical values while the component-removed rows retain their
  original numerical values, Panel B is descriptive rather than a controlled
  matched-retraining effect analysis. ESM-2 and cross-attention show the largest
  negative differences in both splits.}
\label{fig:ablation}
\end{figure}

\FloatBarrier

Relative to the displayed full-model row, the ESM-2-free values differ by
$-0.059$ and $-0.137$ AUROC for the random and Target-ID-held-out splits,
respectively, and the cross-attention-free values differ by $-0.040$ and
$-0.079$.  The domain-label and graph-augmentation rows have small mixed
differences ($+0.002/-0.018$ and $+0.004/-0.004$, respectively).  These numbers
should therefore not be interpreted as controlled causal effects of individual
components, and they do not establish structural binding mechanisms or
