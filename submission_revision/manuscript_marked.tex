...
\begingroup\color{red}$\mathbf{e}_{vu}$ denotes the raw 16-dimensional edge
feature vector, and $\widetilde{\mathbf{e}}_{vu}$ is its learned 16-to-256
projection before message passing. The released \texttt{GINEConv} instantiation
uses the default fixed $\epsilon=0$.\endgroup{} Each layer incorporates batch normalisation, ReLU activation,
...
No model is retrained for the current-release metrics. The canonical
reconstruction instantiates each released checkpoint from its embedded model
configuration, performs full-precision inference, selects an F1 threshold once
on the corresponding validation predictions, and applies that frozen threshold
to test predictions. This separation prevents test-set threshold selection.
\endgroup
...
Inequality-qualified values and pairs with valid replicate $K_d$ measurements on
opposite sides of the \SI{30}{nM} boundary were excluded. The positive label
was $K_d<\SI{30}{nM}$, matching the Davis definition. Before replicate
reconciliation, any external pair was excluded when its canonical isomeric
...
claim of scaffold or remote-homology novelty.
...
existing random-validation threshold (0.5959881544), the same ESM-2 model
(\texttt{esm2\_t30\_150M\_UR50D}; 640 dimensions), and only newly extracted
...
source and prediction hashes, are reported in Table~S14.
\endgroup
...
\midrule
\begingroup\color{red}BioInteract\endgroup & Cross-Attn  & 0.904 & 0.560 & 0.930 & 0.525 & 0.733 & 0.167 \\
\bottomrule
\end{tabular}}
\vspace{4pt}
...
{\color{red}\subsection{Architecture scope and withheld historical ablations}}
...
\begingroup\color{red}Drug-ID-held-out\endgroup & 0.733 & 0.167 \\
\bottomrule
\end{tabular}
\end{table}
...
Instead, it shows limited transfer from this Davis-trained classifier to the
broader BindingDB cohort. This external result does not alter the within-Davis
checkpoint reconstructions and is not used to interpret attention, residue
attribution, binding pockets, or mechanisms.
...
\begingroup\color{red}Fig.~\ref{fig:training} reports a single
checkpoint-matched, contiguous validation-AUROC and training-loss trace for each
archived protocol. The duplicated Random trace and two restarted Target-ID
fragments were excluded because their epoch counters reset or their peak
validation AUROC did not match the released checkpoint. These plots describe
observed optimisation paths only; they do not establish absence of overfitting,
quantify validation-loss behaviour, or estimate repeated-seed variability.\endgroup
...
\caption{\textbf{Training dynamics across splitting protocols.}
  ...
  \begingroup\color{red}Each line is one contiguous archived trace whose peak
  validation AUROC agrees with the saved checkpoint; duplicated and restarted
  segments are not plotted. No fitted or interpolated trends are shown. The
  trajectories do not establish absence of overfitting, validation-loss
  behaviour, or repeated-seed variability.\endgroup}
\label{fig:training}
\end{figure}
...
normalisation threshold. \begingroup\color{red}Figure~\ref{fig:sparsity} is
computed directly from the archived 1{,}301{,}380 pair-normalised residue scores,
not from summary bars; its top 1\% and 5\% of scores account for 78.9\% and
90.4\% of their total attribution mass, respectively.\endgroup{} This describes
concentration of model attribution; it does not demonstrate the size or location
...
\begingroup\color{red}Top 1\% / top 5\% cumulative attribution mass\endgroup & \begingroup\color{red}78.9\% / 90.4\%\endgroup{} \\
Samples analysed            & 1{,}506 \\
\begingroup\color{red}Residue-score observations\endgroup & \begingroup\color{red}1{,}301{,}380\endgroup{} \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{\textbf{Attention sparsity analysis.}
  (A)~Histogram of the archived \begingroup\color{red}pair-normalised
  per-residue attention scores\endgroup{} across all 1{,}506 positive Davis
  pairs (\begingroup\color{red}1{,}301{,}380 observations; logarithmic axes\endgroup{});
  the dashed line denotes the 0.1 sparsity threshold. (B)~\begingroup\color{red}Cumulative
  attribution mass after sorting those same scores in descending order\endgroup{}.
  The \SI{99.2}{\percent} value is an attribution concentration statistic, not a
...
\begingroup\color{red} In particular, the strict external BindingDB test shows limited transfer of the
frozen Davis checkpoint, rather than establishing broad DTI transfer.\endgroup
...
(Table~S13); and strict BindingDB external-validation provenance and results
(Table~S14).
\endgroup
...
publication.\cite{davis2011comprehensive}
\begingroup\color{red}The external evaluation uses the official BindingDB
curated-articles TSV snapshot \texttt{BindingDB\_BindingDB\_Articles\_202607},
obtained from the BindingDB download service;\cite{gilson2016bindingdb} the raw
archive remains available from its source, and the release provides the
deterministic curation and frozen evaluation scripts plus manifest hashes.\endgroup{}
...
\url{https://doi.org/10.5281/zenodo.21436351}. The BioInteract source repository
...
as the immutable Zenodo release, \url{https://doi.org/10.5281/zenodo.21436351}.
The interactive web
