...
\begin{table}[H]
\centering
\caption{Training provenance for the historical entity-split checkpoints.}
\small
\begin{tabular}{p{0.31\linewidth}p{0.61\linewidth}}
\toprule
Setting & Value \\
\midrule
Checkpoint-embedded evidence & Model architecture only; the three entity-split checkpoints do not retain optimizer, scheduler, warmup, or loss state \\
Random and Drug-ID logs & Final-run headers record weighted BCE, label smoothing 0.05, AdamW ($10^{-4}$, $10^{-5}$), and cosine warmup \\
Target-ID log matched to its checkpoint & Older \texttt{run\_split.py} header; only loss and validation AUROC are logged, so optimizer, warmup, and smoothing are not recoverable from the artifact \\
Shared data provenance & Davis binary high-affinity task; seed 42; archived 0.70/0.10/0.20 split proportions \\
Current generic workflow & \texttt{src.cli.train} is a new-run entrypoint, not historical provenance for the released checkpoints \\
Threshold protocol & Validation F1 threshold is frozen before each canonical test evaluation \\
\bottomrule
\end{tabular}
\end{table}
...
the sole source of the Figure~5 distribution and cumulative curve.

\begin{center}
\small
\textbf{Exact summary of the archived pair-normalised residue-attribution array used for Figure~5.}\smallskip
\begin{tabular}{lr}
\toprule
Quantity & Value \\
\midrule
...
Table~S14 records the frozen external evaluation requested by the reviewers.
...
ligand or target were excluded. Confidence intervals are pair-stratified
bootstrap 95\% intervals from 600 replicates (seed 42).}
...
Source rows / final pairs & 93,712 / 1,932 \\
Final positives / negatives & 516 / 1,416 \\
Unique ligands / targets & 1,195 / 396 \\
Exclusions & 82,988 censored/non-exact $K_d$; 8,261 multi-chain; 139 invalid sequences; 8 invalid SMILES; 64 Davis-ligand matches; 94 Davis-sequence matches; 15 threshold-conflicting pairs \\
