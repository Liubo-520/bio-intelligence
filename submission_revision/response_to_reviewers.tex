...
Methods on pp.~3--5; Experimental Setup on pp.~6--7; Results on pp.~7--12;
Discussion on p.~13; Conclusion on pp.~14--16; and availability statements on
p.~16. In the Supporting Information, model/training and environment details are
...
drug-side signal and target relatedness with controls and identity-stratified
  results; (iv) retains only a non-numerical representational contrast with
  published baselines; and (v) redefines attention and Grad-CAM as
...
\begin{itemize}
  \item Figure~1 now presents the canonical entity-split reconstruction as Panel
  ...
  been withdrawn.
  \item Cross-model baseline numbers, their figure, and their duplicated
  Supporting Information table were removed because matched code, splits,
  predictions, and evaluation artefacts are unavailable. Table~1 now provides
  only a non-numerical representational contrast with the cited methods.
...
Supporting Information Tables~S3--S7 and S13; Figs.~1--5 and captions.}
...
\url{https://doi.org/10.5281/zenodo.21436919}. The archive contains the released
...
new Table ``Leakage-aware diagnostic controls'' and revised manuscript Fig.~2;
...
\change{Methods, ``Data Splitting Protocols and Similarity Audit''; Results,
new diagnostic-control table and revised manuscript Fig.~2; Discussion,
...
confirm absence of overfitting. Following final log audit, Figure~3 was rebuilt
...
\change{Results, ``Training Dynamics'' and Figure~3 caption.}
...
of ESM-2. We therefore removed all cross-model numerical baseline values and
the associated figure, rather than retain values without matched code, split
files, predictions, and evaluation artefacts. Table~1 now gives only a neutral
representational contrast with the cited methods. Unequal target pretraining
remains an explicit limitation, and no unperformed ESM baseline is claimed.

The historical internal ablations were likewise withdrawn because matching
ablated-model checkpoint/prediction artifacts are unavailable. The revision
therefore makes no architecture-specific performance claim from unreproducible
comparisons; all reported numbers are traceable BioInteract evaluations or
explicit diagnostic controls.

\change{Results, ``Comparison with Baseline Methods'' and ``Architecture scope
and withheld historical ablations''; Table~1; Discussion.}
...
models. We do not claim to have reproduced additional baselines without matching
their task definition, preprocessing, and pretraining. Because the historical
cross-model values likewise lack matched executable artefacts, we removed them
rather than frame them as a current state-of-the-art leaderboard. A fair
comparison requires a common protocol and matched pretrained target features,
which are outside the evidence available in this revision.
...
be conflated. Figure~5 is described as the
...
\change{Data Availability; Figure~5 caption; Conclusion.}
...
cold-start evaluation'' and revised Fig.~1.}
...
``The 99.2\% Attention Sparsity''; Figure~4 and functional-group tables.}
...
\change{Revised Fig.~1 and caption.}
...
\response Figure~3 now identifies Panel~A as validation AUROC and Panel~B as
...
\change{Revised Figure~3 and caption.}
...
\response We rebuilt Figure~4 from the archived 1,301,380 pair-normalised
...
physical-contact or hotspot labels. Revised Fig.~5 now presents Panel A as the
...
\url{https://doi.org/10.5281/zenodo.21436919}, which preserves the release
...
\begin{itemize}
  ...
  \item Removed all cross-model numerical baseline values, the associated figure,
  and the duplicated Supporting Information table because their matched
  executable artefacts are unavailable; retained only a non-numerical
  representational contrast table.
