...
Page references below use the compiled clean manuscript dated 18 July 2026.
The Abstract and Introduction are on pp.~1--2; Related Work on pp.~2--3;
Methods on pp.~3--7 (fixed-graph configuration p.~4, attribution definitions
pp.~5--6, split audit and software details pp.~6--7); baseline comparison on
pp.~7--8; ablation and split results on pp.~9--11; training and attribution
results on pp.~12--14; Discussion and Conclusion on pp.~14--16; and availability
statements on p.~17. In the Supporting Information, software/split details are on
pp.~2--3, Table~S8 and the reproducibility protocol are on pp.~4--6, and the
remaining input/case/parameter audits are on pp.~6--7. This guide supplements
the section-level locations provided after each response.
...
limits.  The revised manuscript now (i) distinguishes the original
target-identifier-held-out result from sequence-novel transfer; (ii) adds exact
sequence grouping, exact-sequence-grouped cold-target and
exact-sequence-grouped cold-both tests; (iii) quantifies
drug-side signal and target relatedness with controls and identity-stratified
results; (iv) states that the historical comparison is not a matched-pretraining
state-of-the-art comparison; and (v) redefines attention and Grad-CAM as
...
figure. Because corresponding ablated-model artifacts are unavailable, we
withdrew numerical ablation claims rather than compare those historical values
with the reconstructed full-model result.

We also completed a final manuscript--code reconciliation and consistency audit:
\begin{itemize}
  \item Figure~1 now treats the two random-split metric plots jointly as Panel A
  and the identifier-based heatmap as Panel B; the latter uses
  ``Target-ID-held-out'' and ``Drug-ID-held-out'' consistently.
  ...
  validation-selected frozen threshold. Legacy unreproducible result artifacts
  were retired. Because no ablated-model checkpoint/prediction artifacts are
  available, the ablation table, figure, and component-effect conclusions have
  been withdrawn.
  \item The baseline comparison is now explicitly descriptive: causal
  ``Architectural Question'' wording, the gain row, relative-gain emphasis, and
  winner formatting were removed. Supporting Information Table~S8 identifies the
  original submitted comparison table as the numerical source and summarises the
  protocol-compatibility limits of the retained comparison.
...
\change{Abstract; Introduction; Related Work; Methods, ``Model Configuration and
Training'', ``Attention Sparsity'', ``Grad-CAM Functional-Group Attribution'',
and ``Data Splitting
Protocols and Similarity Audit''; Results, ``Comparison with Baseline Methods''
and ``Architecture scope and withheld historical ablations''; Discussion;
Supporting Information Tables~S4, S6--S8; Figs.~1--7 and captions.}
...
\url{https://doi.org/10.5281/zenodo.21429673}. The archive contains the released
code, pretrained weights, configuration files, canonical prediction CSVs,
provenance hashes, figure-generation scripts, and reproduction instructions;
...
limitation transparently, and Supporting Information Table~S10 records every
...
\response The ablation figure was redrawn with separated panel labels, a
non-overlapping legend, readable ticks and values, and an explicit colour scale.
The caption now identifies Panels A/B and limits the ablation interpretation to
the stated architecture and original Target-ID-held-out partition. The protein
channel is now labelled ``domain-label channel''/``unknown-label embedding
channel'' rather than ``domain features.'' The unreconciled ``w/o Graph
Augment.'' row was removed after comparison with the fixed-graph final
configuration.

\change{Revised Fig.~2 and caption.}
...
\response We agree.  We removed the assertion that the plotted single-seed
training curves confirm absence of overfitting.  The revised text says exactly
what the figure contains: archived validation-AUROC and training-loss
trajectories with early stopping.  It explicitly states that these trajectories
do not quantify validation loss or repeated-seed variability and do not isolate
individual regularisation effects.

\change{Results, ``Training Dynamics'' and the caption of former Fig.~4 (now
Fig.~5).}
...
Related Work now discusses GEFA, PGraphDTA, and HCAF-DTA and states explicitly
that BioInteract does not claim first use of a protein language model, graph
encoder, or cross-attention in DTI/DTA prediction.  The contribution is now
defined narrowly as the submitted integration of chemistry-aware molecular
graphs, residue-level ESM-2/physicochemical features with a configured
unknown-label embedding channel, and model-native attribution
outputs for Davis binary classification.  We also clarify that the attributions
are not independently validated structural evidence.
...
\response We agree that the historical comparison does not isolate the effect
of ESM-2.  We therefore removed any claim that it is a fair comparison with all
contemporary ESM-based methods. The six-method table and Fig.~1 now reproduce
the values from the baseline comparison table in the originally submitted
manuscript. Unequal target pretraining remains an explicit limitation. The
historical internal ablations were withdrawn because matching ablated-model
checkpoint/prediction artifacts are not available; we do not use them as
architecture-specific evidence. No unperformed ESM baseline is claimed.

We further removed the ``Gain over best baseline'' row, relative-gain language,
and bold/underlined winner formatting. Table~1 now uses the neutral header
``Representational contrast with BioInteract.'' Supporting Information
Table~S8 identifies the original submitted comparison table as the numerical
source and records the reported split and threshold context,
protein-representation mismatch, and the limits of the comparison.

\change{Results, ``Comparison with Baseline Methods'' and ``Architecture scope
and withheld historical ablations''; Table~1 and Fig.~1; Supporting Information
Table~S8; Discussion.}
...
\response We agree.  We retained Davis because it is the submitted controlled
benchmark, but added strict within-Davis diagnostics and removed any
protein-family-agnostic or broad-pocket generalisation claim.  The revision
does not present those diagnostics as a substitute for BindingDB, KIBA, or other
external datasets.  Broader, consistently preprocessed datasets are now an
explicit requirement for future validation.

\change{Abstract; Methods, split audit; Results, strict tests and controls;
Discussion, ``Scope of Cold-Start Evaluation'' and ``Future Directions''.}
...
models.  We do not claim to have reproduced additional baselines without
matching their task definition, preprocessing, and pretraining.

The revised table is framed only as the submitted comparison set, not as a
current state-of-the-art leaderboard.  A fair update requires a common protocol
and matched pretrained target features, which are outside the evidence available
in this revision.
...
the reproducible input audit is supplied in Table~S10.
...
\response We agree that the cases are illustrative rather than validation.
All case-study language now describes observed model behaviour or a
failure-mode example, and the Conclusion does not use them to establish
a mechanistic conclusion. Their evidential boundary is stated alongside the need
for broader datasets and structural validation.

\change{Results, all interpretability case studies; Discussion; Conclusion.}
...
\response We agree with these priorities. The revision narrows the novelty claim,
labels baseline comparison limitations, treats Davis-only analysis as a
within-benchmark study, and removes structural or mechanistic interpretation
claims that are not independently validated.

\change{Related Work; Results, baseline comparison and split audit; Discussion;
Conclusion.}
...
be conflated. Former Figure~12 (now Fig.~7) is described as the
...
\change{Data Availability; former Fig.~12 (now Fig.~7) caption; Conclusion.}
...
is the original submitted benchmark used for the reported baseline comparison;
it was not selected as a demonstration of protein-family-agnostic performance.
All such language has been removed. We add strict within-Davis tests and controls
while making clear that they are not external validation.
...
\change{Abstract; Methods, ``Configured Domain-Label Channel and Availability'';
Results, ablation interpretation; Supporting Information; Discussion and
Conclusion.}
...
train/test boundary. We label that protocol Target-ID-held-out. We
additionally provide the stricter full-length global-identity audit in R1.2; no
sequence-novel claim is made.

\change{Methods, ``Data Splitting Protocols and Similarity Audit''; Results,
split-audit text and Fig.~3.}
...
architecture, and seed. The 70/10/20 ratios are applied independently to drug
...
``The 99.2\% Attention Sparsity''; current Fig.~6 and functional-group tables.}
...
\response The kinase-only Davis data do not test allosteric or cryptic pockets,
so no robustness claim is made.  The Future Directions now identifies curated
allosteric/cryptic benchmarks and the need for structural or dynamic information
as requirements for that question.
...
\response We initially redrew the ablation figure for readability. However, the
final reproducibility audit found no archived checkpoints or prediction files
for the ablated variants, while the legacy full-model prediction files were not
reconstructible from the released checkpoint and inputs. We therefore removed
the ablation figure, table, and component-effect claims rather than present a
visually improved but irreproducible comparison. The revised manuscript now
states this limitation explicitly and confines conclusions to the reconstructible
full-model and diagnostic-control analyses.

\change{Results, ``Architecture scope and withheld historical ablations''.}
...
\response The caption now identifies Panel A as validation AUROC and Panel B as
training loss and limits their interpretation to observed single-seed
trajectories.

\change{Former Fig.~4 (now Fig.~5) and caption.}
...
\response We moved the legend outside the plotted data region and revised the
caption to define the attribution-concentration statistic.

\change{Former Fig.~5 (now Fig.~6) and caption.}
...
\response The former single-panel Figure~6 (the ABL1--D0017 cross-attention
heatmap) was removed after the input audit showed that the nominal ABL1 variant
analysis was not supportable. It therefore no longer carries a panel label or a
caption in the revised manuscript.

\change{Removed former Fig.~6 as part of the ABL1 input-audit revision.}
...
ABL1 input-audit limitation and Supporting Information Table~S10.}
...
SMILES, protein sequence, high-affinity-class probability, and threshold-distance
category.  The lower portion of the screenshot also shows the generated
...
\change{Former Fig.~12 (now Fig.~7) and caption.}
...
\url{https://doi.org/10.5281/zenodo.21429673}, which preserves the release
...
\begin{itemize}
  ...
  inputs under a frozen validation-threshold protocol, retired unreproducible
  historical result artifacts, and withdrew numerical ablations lacking matching
  checkpoint/prediction artifacts.
  \item Reframed the baseline table as a descriptive historical comparison and
  added a Supporting Information Table~S8 that identifies the original submitted
  comparison table as the source of the retained baseline values.
