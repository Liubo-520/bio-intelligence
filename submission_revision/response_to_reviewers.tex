...
drug-side signal and target relatedness with controls and identity-stratified
  results; (iv) restores the numerical baseline comparison and component-removal
  ablation results, updates the BioInteract full-model entries to the canonical
  current-release metrics, and limits the resulting component differences to
  descriptive evidence; and (v) redefines attention and Grad-CAM as
...
figure.  For the restored component-removal table, we now use those same
canonical current-release values for the displayed full BioInteract row, while
retaining the component-removed rows exactly as reported in the original
submission.  Figure~2 is recalculated from the displayed rows.  This is a
transparent numerical reconciliation, not a new controlled ablation: its
row-to-row differences are described as component sensitivities and are not used
as causal matched-retraining estimates.
...
\begin{itemize}
  ...
  \item The original numerical component-removed rows are retained, while the
  displayed full BioInteract row is updated to the canonical current-release
  metrics. Figure~2 is regenerated from these displayed values, and the text
  explicitly limits its mixed-row differences to descriptive evidence rather
  than controlled component-effect estimates.
...
carry the same information. To make the numerical reconciliation explicit, the
displayed full BioInteract row uses the current canonical entity-split metrics;
the component-removed rows retain their original submitted values. Accordingly,
Panel~B is labelled as a descriptive difference from the displayed full-model
row, not as a matched-retraining causal-effect plot.
...
For the restored component-removal values, the full BioInteract row now uses the
same canonical current-release entity-split metrics that appear in Table~1 and
Figure~1, while the component-removed rows retain their original submitted
numbers. We regenerate Figure~2 from the resulting displayed table, disclose the
mixed provenance in its caption and text, and do not interpret the differences as
controlled causal effects of ESM-2, cross-attention, domain labels, or graph
augmentation. This avoids both an undocumented numerical inconsistency and an
unsupported attribution of performance to a single architectural component.
...
\begin{itemize}
  ...
  component-removal rows; the BioInteract full-model row uses canonical current
  release metrics, Figure~2 is regenerated from the displayed rows, and the
  component differences are explicitly limited to descriptive rather than
  controlled matched-retraining evidence. The baseline comparison remains
  benchmark context rather than a causal architecture comparison.
